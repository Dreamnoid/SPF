...
extern "C"
{
	DLLExport void SetTextureFiltering(SPF::ResourceIndex texture, int filtering)
	{
		return SPF::mTextures.SetFiltering(texture, filtering);
	}

	DLLExport SPF::ResourceIndex LoadTexture(unsigned char* buffer, int length)
	{
		return SPF::mTextures.Load(buffer, length);
	}

	DLLExport void DeleteTexture(SPF::ResourceIndex texture)
	{
		SPF::mTextures.Delete(texture);
	}

	DLLExport int GetTextureWidth(SPF::ResourceIndex texture)
	{
		return SPF::mTextures.GetWidth(texture);
	}

	DLLExport int GetTextureHeight(SPF::ResourceIndex texture)
	{
		return SPF::mTextures.GetHeight(texture);
	}
}