...
namespace SPF
{
	...
	void Window::Open(const char* title, int w, int h)
	{
		...
		mOpenGLContext = SDL_GL_CreateContext((SDL_Window*)mWindow);
	...
	{
		SDL_SetWindowFullscreen((SDL_Window*)mWindow, fullscreen ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
	}

	void Window::SetWindowSize(int w, int h)
	{
		SDL_SetWindowSize((SDL_Window*)mWindow, w, h);
		mWindowWidth = w;
		mWindowHeight = h;
	}
}
...
extern "C"
{
	...
	{
		SPF::mWindow.Open(title, w, h);
	}

	DLLExport int BeginLoop(float* dt)
	{
		return SPF::mWindow.BeginLoop(dt) ? 1 : 0;
	}
	...
	{
		SPF::mWindow.SetFullscreen(fullscreen);
	}

	DLLExport int GetWindowWidth()
	{
		return SPF::mWindow.GetWidth();
	}

	DLLExport int GetWindowHeight()
	{
		return SPF::mWindow.GetHeight();
	}

	DLLExport void SetWindowSize(int w, int h)
	{
		SPF::mWindow.SetWindowSize(w, h);
	}
